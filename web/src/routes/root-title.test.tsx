...
const data = { bridge: "connected", device: undefined, agents: [], shellPanes: [], workspaces: [], tabs: [], sessions: [], session: undefined, snoozedUntil: null, update: undefined, error: false, authError: false, files: false } as HomeData;
...
function show() { const router = createMemoryRouter([{ id: ROOT_ROUTE_ID, path: "/", loader: () => data, element: <RootLayout />, children: [{ index: true, element: <div /> }] }]); return render(<RouterProvider router={router} />); }
...
describe("desktop document title", () => {
  ...
  it("counts needs agents when on", async () => { data.agents = [agent("a"), agent("b")]; setDesktop(true); show(); await waitFor(() => expect(document.title).toBe("(2) Collie")); });
});
