...
// replaces the TUI viewport with thinking; the journal is the session the terminal cannot hold.
// There is no seam-cutter and the live tail is never hidden — the newest turn may show twice.
  ...
  return (
    <div className={desktop ? "flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-hidden" : "flex min-h-0 w-full min-w-0 max-w-[100dvw] flex-1 flex-col overflow-x-hidden"}>
      ...
      <AppHeader
        ...
        //
        // History opens the dedicated transcript page (find / jump-to-turn). The last turns also sit
        // above the live tail so a swipe up reads the conversation without leaving. Offered only when
        // the pane reported an agent session id, so the button never leads to an empty screen.
        //
          ...
          >
            <>
              {/* Agent panes: last transcript turns sit above the live tail so a swipe up reads
                  the conversation. The live TUI always stays — no seam, no hide. Shells (primary
                  screen, real scrollback ring) still page the terminal buffer with Load older. */}
