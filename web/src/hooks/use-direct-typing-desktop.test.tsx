...
beforeEach(() => { localStorage.clear(); setLocked(false); });
afterEach(() => { cleanup(); setLocked(false); });

describe("useDirectTyping desktop", () => {
  it("keeps phone draft refusal and status", () => { render(<><output data-testid="status" /><Probe desktop={false} draft="draft" /></>); fireEvent.click(screen.getByText("arm")); expect(screen.getByTestId("active")).toHaveTextContent("false"); });
