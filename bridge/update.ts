...
export interface UpdateMonitorDeps {
  /** The `owner/name` repo the release check + release links point at (default `bartholomewtj/collie`). */
  repo: string;
...
export class UpdateMonitor {
  ...
  private async runCheck(): Promise<void> {
    ...
    // Two answers, never one (ADR 0025): the newest release the operator can take on a routine
