...
### Changed
- Internal split of `bridge/server.ts` into route-group modules; no behaviour change.
...
### Added
- Files tab Open in browser for HTML documents — unique-origin sandbox, scripts do not run (ba504f1)
...
### Fixed
- Table, box, and highlight rows pan sideways under wrap instead of clipping off the phone; consecutive rows share one scroller so columns stay lined up
...
### Added
- Working status split: blue status dot while an agent runs commands (thinking stays amber)
- Live terminal tail hides while the newest turn is a running command (Show live still pins it), with a running tool row in the transcript
...
### Fixed
- Windows: the Task Scheduler job no longer leaves a console window on screen while the bridge is running (f4347fc, 420e22c, e4afcc2)
...
### Fixed
- Windows: `stop`/`restart`/`start` now stop every bridge running from this checkout (recorded or stray) and wait for the port to come free; `start` refuses — pid on screen — if a foreign listener still holds it, instead of stacking a second bridge on :8787 (#41) (2b2a0a0)
- `scripts/collie-ctl.sh`: the unsupervised path launched the bridge twice per `start` (a merge kept both sides' `nohup` line) — now one, and a repeat `start` stops the previous bridge first; on Git Bash the lifecycle verbs hand over to `contrib/windows/collie-ctl.ps1` (2b2a0a0)
...
- **CI runs with `contents: read` and SHA-pinned actions** — a moved tag can no longer change what CI executes (#12)
- **The systemd unit is confined** — kernel/cgroup/SUID/realtime restrictions, `SystemCallArchitectures=native`, an address-family allow-list and `UMask=0077`; `MemoryDenyWriteExecute` stays off (Bun's JIT) (#12)
- **Generated unit values are quoted** — a checkout or config path containing a space no longer truncates `Environment=` / `ExecStart=` (#12)

### Fixed

- **The pidfile kill matches this checkout's absolute path** — a recycled pid running another checkout's `bridge/index.ts` is no longer killed on `start` (#12)
- **Windows launchers quote each argument** — a path with a space or a quote no longer re-splits into the wrong arguments (#12)
