...
The ADW quality module (`adws/adw_modules/quality.py`) defines a clear split between deterministic test execution (`run_tests()`) and full quality gate enforcement (`run_quality()`), establishing distinct block boundaries, bounded output tails, and isolated execution scopes.
...
- **`versions`** (Python check): Verifies that `herdr-plugin.toml` (canonical), `package.json`, `web/package.json`, and the newest `## [x.y.z]` heading in `CHANGELOG.md` all agree.
- **`version_bump`** (Python check): Enforces that if any functional paths (`bridge/`, `web/src/`, `web/public/`, `scripts/`, manifests) were modified, `herdr-plugin.toml`'s version differs from HEAD's and has not regressed.
...
- **Bounded 4k Tail**: `TAIL_CHARS = 4_000`. Each failing block captures `(stdout + stderr)[-4000:]` independently, preventing a verbose stack trace from overflowing the builder agent's handoff envelope.
...
- **Self-Healing Installs**: `_ensure_installs()` checks for `node_modules` and `web/node_modules`, running `bun install --frozen-lockfile` only when missing (e.g. fresh fleet sandbox bundles).
...
In this finalized architecture:
...
- `app_docs/2645f766_quality-block-split.md`: This documentation.
...
2. Inspect git status to confirm no unauthorized modifications to protected files or version files:
   ...
   git status
   ```
   Only `specs/2645f766_quality-block-split.md` and `app_docs/2645f766_quality-block-split.md` are added.
