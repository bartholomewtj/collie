...
under *Direct surface*) and §6 (the Ctrl+` row only). The spec's **Decisions** table is settled — do
not re-open any row in it.

## What this does

Desktop mode already has its shell, sidebar, hidden phone controls and the Ctrl+Alt+Up/Down pane
hotkeys. This chunk makes the keyboard behave like a desktop keyboard:
...
- **Armed = focused.** The ring on the mirror and the strip over the input follow the textarea's own
  focus. Click anywhere else, alt-tab away, or hit the idle pause, and it releases.
...
| Thing | Value |
|---|---|
| `cd web && bun run test` | 134 files, **2710 passed**, 19 todo |
| `bun run test` (root) | **859** |
| Version in all four files | `0.56.3` |
| Newest CHANGELOG heading | `## [0.56.3] - 2026-08-22` |

Record the same numbers after the change: web goes **up** by exactly the tests you add, root stays
**859**, and **no existing test file is edited**.
...
- the direct-surface **keydown mapper** (`keyForKeyDown` in `use-direct-typing.ts` keeps its current
  six-key table), **modifier chords**, **paste handling** (`onPasteImage` unchanged), and **Ctrl+F**.
  Those are the next chunks of phase 2.
...
| `web/src/hooks/use-direct-typing.ts` | new `desktop` option; draft-refusal bypass in the `on` branch; silent arm/release on desktop; `onBlur`; `window.blur` and idle-cover release; update the header comment. |
| `web/src/components/composer.tsx` | desktop keydown branch (Enter sends, Shift+Enter newline, empty-box pass-through); imperative handle gains `armDirect` / `releaseDirect` / `toggleDirect`; `onArmedChange` prop; strip moves into the input row on desktop; greyed strip when locked. |
| `web/src/components/direct-typing-strip.tsx` | optional `disabled` + `reason` props for the read-only / gone case. |
| `web/src/components/agent-chat.tsx` | mirror ref + armed ring; `focusFromMirror` arms on desktop; window `pointerdown` release; subscribe to the arm-toggle request; track armed for the ring. |
...
Ctrl+` is a **global** chord handled in `useDesktopHotkeys`, which is mounted in `DesktopShell` at
the root of the tree. The armed state lives in `useDirectTyping` inside `<Composer>`, several levels
down. One tiny module bridges them.
...
Do **not** put this in `lib/desktop.ts`: that module persists to localStorage, and the whole point of
the armed state is that it never does.

## 2. `web/src/hooks/use-desktop-hotkeys.ts`

The existing handler opens with `if (event.shiftKey || event.metaKey || !event.ctrlKey ||
!event.altKey) return;`, which throws away every chord without Alt. Ctrl+` must therefore be checked
**before** that line.
...
`@/lib/direct-arm`. `desktopPrefs()` (the live read), not `useDesktop()` — the listener is stable
across renders and must not be re-registered for a pref change.
...
`composer.tsx` already computes `const desktop = useDesktop().on;` — pass it straight in. Do not call
`useDesktop()` inside the hook; the caller owning the environment keeps the hook testable without the
store.

### 3.2 `activate()` — the draft bypass

Keep the refusal; branch it. Do **not** delete it:
...
- **`window.blur`** — effect keyed on `[desktop, active]`, mounted only while `desktop && active`:
  `window.addEventListener("blur", releaseOnFocusLoss)`. **Capture must be `false`**: `blur` does not
  bubble, but a capturing window listener *does* see element blurs, and that would double up with
  `onBlur` and fire on unrelated fields.
...
Do not touch the existing `suspended`, visibility, pane-change or failed-batch disarms.
...
choice — the 'Type' toggle in the composer's Controls row, beside Keys."*, add a desktop paragraph:
...
// pref, and clicking the terminal or pressing Ctrl+` is what arms the surface you already chose.
// The surface PREF is stored (lib/desktop.ts, localStorage). The ARMED STATE still is not, and is
// not lifted anywhere: on desktop it is simply the textarea's own focus — release on blur, on a
// pointerdown outside the mirror, on window blur, on the idle cover. Nothing survives a reload.
```
...
- `disabled: true` → muted colours (`text-muted-foreground`), **no Stop button**, and the trailing
  text is `— {reason}` instead of `— keys go straight through`. This is the read-only-device / gone-
  pane state: the strip still says what the surface is, and says why it cannot capture.
- `disabled: false` → exactly today's markup.

Keep the leading label text `Typing into terminal` identical in both states so one selector finds it.
...
(deliberately not depending on the callback identity; agent-chat passes a `useState` setter, which is
stable).

### 5.2 Pass `desktop` to the hook

`useDirectTyping({ ..., desktop })`. `const desktop = useDesktop().on;` already exists above it — move
that line above the `useDirectTyping` call if it is not already.

### 5.3 Imperative handle — **read the trap first**

`useImperativeHandle(ref, () => ({ ... }), [])` today closes over `focusInputImmediately`, which only
touches refs, so `[]` deps are safe. `direct` is a **fresh object every render**, and `direct.activate`
closes over `canActivate` → `locked`, `sending`, `uploading` from *that* render. Capturing it under
`[]` deps would freeze the first render's values and silently arm over a read-only pane.
...
`ComposerHandle` gains the three methods, each with a one-line doc comment.
...
(Same body as `toggleDirect`; factor it into one local `function toggleDirect()` reading `directRef`
and use it from both places.)

### 5.5 The keydown branch

Replace the non-armed arm of the existing ternary. The armed arm (`direct.onKeyDown`) is unchanged.
...
```ts
// Herdr's `+`-joined key grammar, not tmux: `Up`, not `C-Up`. See HERDR_API.md.
...
`pressKeys` already returns early when `locked`, and already schedules the revalidate on success.

### 5.6 `onBlur` on the input

`<ChatInput ... onBlur={direct.onBlur} />`. No-op unless `desktop && active`.

### 5.7 The strip moves into the input row on desktop

Two renderings now:

- **Phone (`!desktop`)** — unchanged: `{!desktop && direct.active && <DirectTypingStrip … />}` stays
...
`typing` comes from the same `useDesktop()` call: `const { on: desktop, typing } = useDesktop();`.
...
And suppress the Send button while `showDesktopStrip` — the strip's Stop is the release control, and
a button left tabbable behind an opaque overlay is a keyboard trap. Wrap the existing
`{!direct.active && forcingSend ? … : … }` chain in `{!showDesktopStrip && ( … )}`.

`opacity-0` (not `hidden`, not `sr-only`): the field keeps its box, so the row does not jump, and it
stays focusable. Do not add `pointer-events-none` — the overlay already covers it.

## 6. `web/src/components/agent-chat.tsx`

### 6.1 Armed state + mirror ref

```ts
const [armed, setArmed] = useState(false);
const mirrorRef = useRef<HTMLDivElement>(null);
const { on: desktop, typing } = useDesktop();   // replaces the existing `const desktop = useDesktop().on`
```

Move the `useDesktop()` line up next to the other hooks (it currently sits just before the return);
`focusFromMirror` needs `typing`.
...
spec sit under *Direct surface*. On the composer surface a mirror click still just focuses, as today.
Ctrl+` is the escape hatch: it flips the pref to `direct` and arms in one press.
...
On the mirror wrapper (currently
...
(`mirrorRef` is not read by the pointerdown rule above — keep it only if you use it; if not, drop it
rather than leaving an unused ref, since `noUnusedLocals` is on.) **Decision: drop `mirrorRef`.** The
release rule is "anything that is not the field", which needs no mirror reference.

### 6.5 Pass the callback

`<Composer ... onArmedChange={setArmed} />`.

## 7. `CLAUDE.md`

Two edits, both small. Doc-only edits do not need a bump — but this commit carries code, so the bump
below applies anyway.

**a. The bullet at ~line 196** — replace the first sentence's entry-point clause and add the desktop
sentence, keeping the rest:

> - **"Type into terminal" is armed by a named choice and dies with the pane view.** On the phone it
>   is the "Type" toggle in the Controls row beside Keys — never a gesture on Send, and never a bare
>   long-press. **In desktop mode the named choice is the "Typing surface" pref in Settings, and a
>   click on the mirror or Ctrl+` arms the surface you already picked; the surface PREF is stored
>   (`lib/desktop.ts`), the ARMED STATE is not — there it is simply the composer textarea's own
>   focus, released by a blur, a pointerdown outside the field, `window.blur` or the idle cover.** It
>   disarms on a pane switch, a composer lock (gone pane, read-only, idle pause), a hidden page, and
>   a failed batch — never persisted, never restored. Don't lift it, and don't add the reply guard's
>   `composerReady` pre-flight to it; …*(rest unchanged)*
...
anything. Set `0.56.4` in `herdr-plugin.toml`, `package.json`, `web/package.json`, and add:
...
Copy the probe/`mount` shape from the existing `use-desktop-hotkeys.test.tsx`.
- Ctrl+` with `typing: "composer"` → `desktopPrefs().typing === "direct"`, the subscriber fired, and
...
Reuse `renderComposer` / `renderComposerWithStatus` from `composer.test.tsx` by copying them (they
are local helpers; copy, do not export from the existing file — that would be editing it).

With `setDesktop(false)` (**the phone-unchanged half**):
- Enter in the composer **inserts a newline** and sends nothing.
...
- Enter with text → one guarded send (assert via the `recordReply` / `replyHandler` protocol already
  used in `composer.test.tsx`), and the event is `defaultPrevented`.
...
- Empty box: Escape / Tab / ArrowUp / ArrowDown / ArrowLeft / ArrowRight each POST
  ...
  `["Right"]`.
- Non-empty box: Escape and ArrowUp send **no** keys (they are editing keys once there is text).
- Empty box, **Ctrl+C** → no `/keys` call and not `defaultPrevented`.
- Send still runs the two-tap destructive confirm: type `rm -rf /tmp/x`, click Send → status matches
  `/Destructive: .* tap Send again/` and no reply went out; click again → it sends.
...
Render `AgentChat` the way `agent-chat-desktop.test.tsx` does, with `setDesktop(true)` and
`setTyping("direct")`.
...
- Clicking the mirror while `window.getSelection()` reports a non-collapsed selection does **not**
  arm. Stub it with `vi.spyOn(window, "getSelection").mockReturnValue({ isCollapsed: false } as
  Selection)`.
- With `setTyping("composer")`, clicking the mirror focuses but does **not** arm.
- Armed → the mirror wrapper carries the ring class (`toHaveClass("ring-2")` on the element found via
  a `data-testid` you add, or via `container.querySelector`).
- `requestArmToggle()` inside `act()` arms; a second call releases (this is the Ctrl+` path end to
  end, without needing `DesktopShell`).
- Armed, then `fireEvent.pointerDown(document.body)` → released (strip gone). Then re-arm and
...
A small probe component that calls the hook directly with a real `<textarea>` and a controllable
`replyDraft`.
...
- `desktop: true`, armed, then `fireEvent.blur(textarea)` (with `onBlur` wired) → released.
...
- `desktop: true`, armed, then `act(() => setLocked(true))` (from `@/lib/idle`) → the textarea is
  blurred and the mode is released. Reset with `setLocked(false)` in `afterEach`.
...
cd web && bun run test          # 134+ files; 2710 + your new tests; 0 failed
cd .. && bun run test           # still 859
bun run typecheck 2>/dev/null || (cd web && bun run typecheck)   # both sides clean
bash scripts/check-version.sh   # prints ✓
git diff --stat                 # no existing *.test.* file appears
```

`web/bun run test` runs `bun run typecheck && vitest run`, so the web typecheck is covered by the
first command. Judge each by its **exit status**, not by scanning output for the word "error".
...
1. **`useImperativeHandle` with `[]` deps captures the first render's `direct`.** Use the
   `directRef` pattern in §5.3 or you will get a handle that arms over a read-only pane.
2. **`window.addEventListener("blur", …, true)`** sees element blurs. Use capture `false`.
3. **`noUnusedLocals` / `noUnusedParameters` are on everywhere**, and `web/` also enforces
   `verbatimModuleSyntax` (`import type { … }`) and `erasableSyntaxOnly`. A stray unused ref fails the
   build, not just the lint.
4. **Never a `dark:` variant inside the mirror `<pre>`** — not touched here, but the ring goes on the
   wrapper, not inside `AnsiOutput`.
5. **Herdr key grammar is `+`-joined, not tmux**: `Up`, `Escape`, `Tab`. `Home`/`End`/`PageUp`/
   `Delete` are unsupported and are deliberately absent from `PASS_THROUGH_KEYS`.
6. The pre-commit hook blocks a functional commit without a bump; `scripts/check-version.sh` is the
   check it runs.
7. Branch, don't commit to `main`: `git checkout -b feat/desktop-typing-composer`, then push and
